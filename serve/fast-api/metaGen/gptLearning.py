...
#计算文本的token数量
def num_tokens_from_string(s,model='gpt-4-0613'):
    encoding=tiktoken.encoding_for_model(model)
    num_tokens=len(encoding.encode(s))
    return num_tokens

#关于function calling自动化流程的函数
#把函数列表翻译成functions参数列表   
...
def get_email_counts(userId='me'):
    """
    查询 Gmail 邮箱中总共有多少封邮件
    :param userId: 必要参数，字符串类型，用于表示需要查询的邮箱 ID，
    注意，当查询我的邮箱时，userId需要输入'me'；
    :return：返回当前邮件总数，结果将于json格式的字符串中返回
    """
    # 导入必要模块
    ...
    # 获取所有邮件数量
    results = service.users().labels().get(userId=userId, id='INBOX').execute()
    email_count = results.get('threadsTotal')

    # 将邮件数量转为 json 对象
    email_count_json = json.dumps({'email_count': email_count})

    return email_count_json
...
def code_generate(req, few_shot='all', model='gpt-4-0613', g=globals(), detail=0, system_messages=None):
    ...
    print('done')
    return function_name
def get_email_chat(userId='me', userEmail=''):
    """
    修改后的函数，该函数用于获取用户自己和特定邮件发件人或收件人之间的所有通信邮件。
    
    :param userId: 可选参数，默认值为 'me'，表示查看我的邮件。userId是用户的邮件地址。 
    :param userEmail: 必要参数，字符串类型，表示需要检索的邮件来源者和接收者。请输入完整的邮箱地址。
    :return: 返回一个json格式的字符串，该字符串包含了查询到的所有邮件内容，如果没有查询到符合条件的邮件，就返回一个空的json对象字符串。
    """
    # 从本地文件中加载凭据
    creds = Credentials.from_authorized_user_file('token.json')
    ...
    # 格式化我们的查询
    query = f"from:({userEmail}) OR to:({userEmail})"
    
    # 使用 Gmail API 中的 list 方法列出满足我们查询条件的所有邮件
    results = service.users().messages().list(
        userId=userId, 
        q=query).execute()
    messages = results.get('messages', [])
...
def function_test(function_name, req, few_shot, model="gpt-4-0613", g=globals(), system_messages=None):
    ...
    def test_messages(ueser_content):
        messages = [{"role": "system", "content": "余露的邮箱地址是:17785629033@163.com"},
                    {"role": "system", "content": "我的邮箱地址是:hjxylgogogo@gmail.com"},
...
def Gmail_auto_func(req, few_shot='all', model='gpt-4-0613', g=globals(), detail=0):
    ...
    # 尝试进行任务拆解
    try:
        decompose_results = get_decompose_results(req=req, model=model) #return list
    except Exception as e:
        print(e)
        print('暂停1分钟后继续调用模型')
        time.sleep(60)
